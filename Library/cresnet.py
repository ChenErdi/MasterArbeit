...
class BasicCResNet(nn.Module):
    ...
    def __init__(self, 
        ...
        super(BasicCResNet, self).__init__()
        ...
        for i in range(self.num_cnn):
            test_output = getattr(self, "Convlayer{}".format(i))(test_output)
            self.intermediate_output_shape.append(
                test_output.shape
            )
            
        #test_output = self.forward_cnn(test_input)
    ...
    def forward_conv(self, x):
        for i in range(self.num_cnn):
            x = getattr(self, "Convlayer{}".format(i))(x)
...
def initialize_cresnet(config, device, weight_fpath=None, use_legacy=False):
